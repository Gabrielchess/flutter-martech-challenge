...
data "aws_iam_policy_document" "sfn" {
  ...
  # Exigido pelo Step Functions para entregar log em CloudWatch. O servico nao
...
# da execucao e a execucao ja termina em FAILED, que e o que o alarme observa.
# Quando existir um topico de alerta, entra um Catch -> Publish -> Fail.
resource "aws_sfn_state_machine" "this" {
  name     = "${var.project}-pipeline"
  role_arn = aws_iam_role.sfn.arn

  definition = templatefile("${path.module}/statemachine.asl.tftpl", {
    fx_arn     = var.lambda_arns["fx"]
    silver_arn = var.lambda_arns["silver"]
    gold_arn   = var.lambda_arns["gold"]
  })
