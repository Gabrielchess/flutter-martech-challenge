    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          region = var.region
          view   = "table"
          query  = "SOURCE '${var.log_group_names["silver"]}' | fields @timestamp, ingest_date, Entity, RowsIngested, RowCount, RowsQuarantined, ChecksTotal, DuplicateKeys, BlankValues, FormatViolations, OutOfRangeNumerics, FutureDatedRecords, ProcessingDurationMs\n| filter ispresent(Entity)\n| sort @timestamp desc\n| limit 30"
        }
      },
      {
        type   = "log"
        ...
        properties = {
          ...
          region = var.region
          view   = "table"
          query  = "SOURCE '${var.log_group_names["silver"]}' | SOURCE '${var.log_group_names["gold"]}' | fields @timestamp, entity, column, check, severity, failed_records, failed_ratio, passed, details\n| filter dq_check = 1\n| sort @timestamp desc, failed_records desc\n| limit 30"
        }
      },
    ]
  })
